...
@section('content')
    ...
    <div class="w-full">
        <form id="edit-form" action="{{ route('edit.jadwal2', ['id' => $jadwal->kode_jadwal]) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="grid md:grid-cols-2 gap-6 p-6">
                    ...
                    <select id="dojo" name="kode_dojo"
                        ...
                        @foreach ($dojo as $item)
                            <option value="{{ $item->kode_dojo }}"
                                {{ $jadwal->kode_dojo == $item->kode_dojo ? 'selected' : '' }}>
                                {{ $item->nama_dojo }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <textarea rows="3" id="catatan" name="catatan"
                        class="py-3 px-4 w-full border border-gray-300 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none ">{{ $jadwal->catatan }}</textarea>
                </div>
